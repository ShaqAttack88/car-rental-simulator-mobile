// ============================================================
// WashBay.js — Individual car wash bay with color combo mechanics
// ============================================================

import { FacilityBase } from './FacilityBase.js';
import { COLOR_MAP } from './ParkingBay.js';

export class WashBay extends FacilityBase {
  constructor(x, y) {
    super('carwash', x, y, 14, 18);
  ...
  triggerCombo(multiplier) {
    ...
    this.sparkles = [];
    const colorInfo = COLOR_MAP[this.assignedColor] || COLOR_MAP.blue;
    for (let i = 0; i < 8; i++) {
  ...
  render(ctx, sprites) {
    ...
    // 2. Soap suds / water bubbles in pavement
    ctx.fillStyle = '#0284c7';
    ctx.fillRect(bx + 3, by + 4, 2, 2);
    ctx.fillRect(bx + 9, by + 6, 2, 2);
    ctx.fillRect(bx + 4, by + 11, 2, 2);
    ctx.fillRect(bx + 8, by + 13, 2, 2);

    ctx.fillStyle = '#bae6fd';
    ctx.fillRect(bx + 3, by + 4, 1, 1);
    ctx.fillRect(bx + 9, by + 6, 1, 1);
    ctx.fillRect(bx + 4, by + 11, 1, 1);
    ctx.fillRect(bx + 8, by + 13, 1, 1);

    // 3. Colored combo borders or neutral water-cyan dashed lines
    if (this.assignedColor && COLOR_MAP[this.assignedColor]) {
      ...
      // Neutral bay: water-blue dashed lines
      ctx.save();
    ...
    // 5. Combo celebration pulse & sparkles
    if (this.comboPulseTimer > 0) {
      const c = COLOR_MAP[this.assignedColor] || COLOR_MAP.blue;
